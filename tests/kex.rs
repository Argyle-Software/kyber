...
fn uake_valid() {
  ...
  let server_send = bob.server_receive(client_init, bob_keys.expose_secret(), &mut rng).unwrap();
...
fn uake_invalid_client_init_ciphertext() {
  ...
  assert!(bob.server_receive(client_init, bob_keys.expose_secret(), &mut rng).is_err());
}
...
fn uake_invalid_server_send_ciphertext() {
  ...
  let bob_keys = keypair(&mut rng);
  let client_init = alice.client_init(&bob_keys.public, &mut rng);
  let mut server_send = bob.server_receive(client_init, bob_keys.expose_secret(), &mut rng).unwrap();
...
fn ake_valid() {
  ...
  let bob_keys = keypair(&mut rng);
  let client_init = alice.client_init(&bob_keys.public, &mut rng);
  let server_send = bob.server_receive(client_init, &alice_keys.public, bob_keys.expose_secret(), &mut rng).unwrap();
  let _client_confirm = alice.client_confirm(server_send, alice_keys.expose_secret()).unwrap();
  assert_eq!(alice.shared_secret, bob.shared_secret);
}
...
fn ake_invalid_client_init_ciphertext() {
  ...
  let mut client_init = alice.client_init(&bob_keys.public, &mut rng);
  client_init[KYBER_PUBLICKEYBYTES..][..4].copy_from_slice(&[255u8;4]);
  assert!(bob.server_receive(client_init, &alice_keys.public, bob_keys.expose_secret(), &mut rng).is_err());
}
...
fn ake_invalid_client_init_publickey() {
  ...
  client_init[..4].copy_from_slice(&[255u8;4]);
  let server_send = bob.server_receive(client_init, &alice_keys.public,  bob_keys.expose_secret(), &mut rng).unwrap();
  assert!(alice.client_confirm(server_send, &alice_keys.expose_secret()).is_err());
}
...
fn ake_invalid_server_send_first_ciphertext() {
  ...
  let bob_keys = keypair(&mut rng);
  let client_init = alice.client_init(&bob_keys.public, &mut rng);
  let mut server_send = bob.server_receive(client_init, &alice_keys.public, bob_keys.expose_secret(), &mut rng).unwrap();
  server_send[..4].copy_from_slice(&[255u8;4]);
  assert!(alice.client_confirm(server_send, &alice_keys.expose_secret()).is_err());
}
