...
fn uake_failed_randombytes_client_init() {
  ...
  let bob_keys = keypair(&mut rng).unwrap();
  let mut rng = FailingRng::default();
  assert_eq!(alice.client_init(&bob_keys.public, &mut rng), Err(KyberError::RandomBytesGeneration));
}
...
fn uake_failed_randombytes_server_receive() {
  ...
  let client_init = alice.client_init(&bob_keys.public, &mut rng).unwrap();
  let mut rng = FailingRng::default();
  assert_eq!(bob.server_receive(client_init, &bob_keys.secret, &mut rng), Err(KyberError::RandomBytesGeneration));
}
...
fn ake_failed_randombytes_server_receive() {
  ...
  let client_init = alice.client_init(&bob_keys.public, &mut rng).unwrap();
  let mut rng = FailingRng::default();
  assert_eq!(bob.server_receive(client_init, &alice_keys.public, &bob_keys.secret, &mut rng),
