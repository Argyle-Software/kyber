    ...
    128 => {
      for i in 0..KYBER_N / 8 {
        for j in 0..8 {
          ...
          t[j] = (((((u as u16) << 4) + KYBER_Q as u16 / 2) / KYBER_Q as u16)
            & 15) as u8;
        }
        r[k] = t[0] | (t[1] << 4);
        r[k + 1] = t[2] | (t[3] << 4);
    ...
    160 => {
      for i in 0..(KYBER_N / 8) {
        for j in 0..8 {
          ...
          u = a.coeffs[8 * i + j];
          u += (u >> 15) & KYBER_Q as i16;
          t[j] = (((((u as u32) << 5) + KYBER_Q as u32 / 2) / KYBER_Q as u32)
            & 31) as u8;
        }
        r[k] = t[0] | (t[1] << 5);
  ...
  match KYBER_POLYCOMPRESSEDBYTES {
    128 => {
      ...
      for i in 0..KYBER_N / 2 {
        r.coeffs[2 * i + 0] =
          ((((a[idx] & 15) as usize * KYBER_Q) + 8) >> 4) as i16;
        r.coeffs[2 * i + 1] =
          ((((a[idx] >> 4) as usize * KYBER_Q) + 8) >> 4) as i16;
        idx += 1;
      }
    }
    160 => {
...
{
  for i in 0..(KYBER_N / 2) {
    r.coeffs[2 * i + 0] = ((a[3 * i + 0] >> 0) as u16
