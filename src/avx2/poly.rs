...
{
  let shift: __m256i = _mm256_broadcastsi128_si256(_mm_set_epi32(0,1,2,3));
  let idx: __m256i = _mm256_broadcastsi128_si256(
  ...
  let j = i as usize; 
  _mm256_store_si256(&mut r.vec[0+2*j+0],g0);
  _mm256_store_si256(&mut r.vec[0+2*j+1],g1);
  _mm256_store_si256(&mut r.vec[8+2*j+0],g2);
  _mm256_store_si256(&mut r.vec[8+2*j+1],g3);
  };
