...
use crate::{
  align::*, params::*, poly::*, polyvec::*, rejsample::*, symmetric::*,
  CryptoRng, RngCore,
};
#[cfg(not(feature = "90s"))]
...
unsafe fn gen_matrix(a: &mut [Polyvec], seed: &[u8], transposed: bool)
{
  ...
  while ctr0 < KYBER_N || ctr1 < KYBER_N || ctr2 < KYBER_N || ctr3 < KYBER_N {
    ...
    ctr0 += rej_uniform(
      &mut a[0].vec[0].coeffs[ctr0..],
      KYBER_N - ctr0,
      &buf[0].coeffs,
      SHAKE128_RATE,
    );
    ctr1 += rej_uniform(
      &mut a[0].vec[1].coeffs[ctr1..],
      KYBER_N - ctr1,
      &buf[1].coeffs,
      SHAKE128_RATE,
    );
    ctr2 += rej_uniform(
      &mut a[1].vec[0].coeffs[ctr2..],
      KYBER_N - ctr2,
      &buf[2].coeffs,
      SHAKE128_RATE,
    );
    ctr3 += rej_uniform(
      &mut a[1].vec[1].coeffs[ctr3..],
      KYBER_N - ctr3,
      &buf[3].coeffs,
      SHAKE128_RATE,
    );
...
#[cfg(all(
  not(feature = "kyber512"),
  not(feature = "kyber1024"),
  not(feature = "90s")
))]
unsafe fn gen_matrix(a: &mut [Polyvec], seed: &[u8], transposed: bool)
{
  ...
  while ctr0 < KYBER_N || ctr1 < KYBER_N || ctr2 < KYBER_N || ctr3 < KYBER_N {
    ...
    ctr2 += rej_uniform(
      &mut a[0].vec[2].coeffs[ctr2..],
      KYBER_N - ctr2,
      &buf[2].coeffs,
      SHAKE128_RATE,
    );
    ctr3 += rej_uniform(
      &mut a[1].vec[0].coeffs[ctr3..],
      KYBER_N - ctr3,
      &buf[3].coeffs,
      SHAKE128_RATE,
    );
  ...
  while ctr0 < KYBER_N || ctr1 < KYBER_N || ctr2 < KYBER_N || ctr3 < KYBER_N {
    ...
    ctr0 += rej_uniform(
      &mut a[1].vec[1].coeffs[ctr0..],
      KYBER_N - ctr0,
      &buf[0].coeffs,
      SHAKE128_RATE,
    );
    ctr1 += rej_uniform(
      &mut a[1].vec[2].coeffs[ctr1..],
      KYBER_N - ctr1,
      &buf[1].coeffs,
      SHAKE128_RATE,
    );
    ctr2 += rej_uniform(
      &mut a[2].vec[0].coeffs[ctr2..],
      KYBER_N - ctr2,
      &buf[2].coeffs,
      SHAKE128_RATE,
    );
    ctr3 += rej_uniform(
      &mut a[2].vec[1].coeffs[ctr3..],
      KYBER_N - ctr3,
      &buf[3].coeffs,
      SHAKE128_RATE,
    );
...
unsafe fn gen_matrix(a: &mut [Polyvec], seed: &[u8], transposed: bool)
{
  ...
  for i in 0..4usize {
    ...
    while ctr0 < KYBER_N || ctr1 < KYBER_N || ctr2 < KYBER_N || ctr3 < KYBER_N {
      ...
      ctr0 += rej_uniform(
        &mut a[i].vec[0].coeffs[ctr0..],
        KYBER_N - ctr0,
        &buf[0].coeffs,
        SHAKE128_RATE,
      );
      ctr1 += rej_uniform(
        &mut a[i].vec[1].coeffs[ctr1..],
        KYBER_N - ctr1,
        &buf[1].coeffs,
        SHAKE128_RATE,
      );
      ctr2 += rej_uniform(
        &mut a[i].vec[2].coeffs[ctr2..],
        KYBER_N - ctr2,
        &buf[2].coeffs,
        SHAKE128_RATE,
      );
      ctr3 += rej_uniform(
        &mut a[i].vec[3].coeffs[ctr3..],
        KYBER_N - ctr3,
        &buf[3].coeffs,
        SHAKE128_RATE,
      );
...
pub fn indcpa_enc(c: &mut [u8], m: &[u8], pk: &[u8], coins: &[u8])
{
  unsafe {
    ...
    {
      const NOISE_NBLOCKS: usize = (KYBER_ETA1 * KYBER_N / 4) / XOF_BLOCKBYTES;
      const CIPHERTEXTNOISE_NBLOCKS: usize =
        (KYBER_ETA2 * KYBER_N / 4) / XOF_BLOCKBYTES;
